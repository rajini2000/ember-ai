...
static volatile int btn_tap_count = 0;
static volatile bool btn_reset_pending = false;
static volatile bool btn_alarm_dismiss = false;
static Timer btn_timer;
static bool btn_timer_started = false;
...
// ========== M4: Remote Config Variables ==========
static bool alarm_armed = true;              // Can be disarmed from web panel
static bool test_alarm_requested = false;    // Web panel test button
...
// Send sensor data to AI server via ESP32 HTTPS POST, returns parsed result
...
int main() {
    ...
    while (true) {
        ...
        // --- Smart button handling: 1=page, 3=calib, 5=setup ---
        if (btn_pressed && !btn_timer_started) {
            btn_pressed = false;
            btn_tap_count = 1;  // This press is tap #1
            btn_timer.reset();
            btn_timer.start();
            btn_timer_started = true;
        }
        if (btn_timer_started) {
            // Collect taps for 700ms
            if (btn_timer.elapsed_time() < 700ms) {
                poll_button();
                if (btn_pressed) {
                    btn_pressed = false;
                    // tap already counted by poll_button
                }
                // Don't sleep the full 2s, poll quickly
                ThisThread::sleep_for(50ms);
                continue;
            }
            // Timer expired - evaluate tap count
            btn_timer.stop();
            btn_timer.reset();
            btn_timer_started = false;
            int taps = btn_tap_count;
            btn_tap_count = 0;
            btn_pressed = false;

            char tap_msg[48];
            snprintf(tap_msg, sizeof(tap_msg), "[BTN] %d taps detected\r\n", taps);
            pc_print(tap_msg);

            if (taps >= 5) {
                btn_reset_pending = true;
            } else if (taps >= 3 && alarm_active) {
                // 3+ presses while alarm: dismiss alarm
                btn_alarm_dismiss = true;
            } else if (taps >= 3) {
                // 3+ presses (no alarm): start sensor calibration
                pc_print("\r\n[BTN] 3 taps -> starting sensor calibration\r\n");
                ember_calibration_trigger();
            } else {
                // 1-2 presses: switch page
                oled_page = (oled_page + 1) % OLED_PAGES;
                oled_update(); // immediate visual feedback
            }
        }

        // --- 3-press: dismiss alarm and reset ---
        if (btn_alarm_dismiss) {
            ...
            if (fire_alert_active) {
                ...
                pc_print("\r\n[FIRE] Fire alert dismissed by 3-press\r\n");
            }
        ...
        // ========== M4: Poll remote config from web control panel ==========
        if (wifi_connected && !softap_mode) {
            pollRemoteConfig();
        }

        // ========== M4: Handle test alarm (3s burst) ==========
        ...
        // --- 5-press button reset: enter SoftAP setup mode ---
        if (btn_reset_pending) {
            btn_reset_pending = false;
            pc_print("\r\n[RESET] 5 presses detected - entering WiFi setup...\r\n");
            alarm = 0;
            if (oled_available) oled_cmd(0xA6); // ensure normal display

            // Disconnect current WiFi
            char r[256];
            esp_send_cmd("AT+CWQAP", r, sizeof(r), 3000);
            ThisThread::sleep_for(500ms);
            wifi_connected = false;

            // Generate new PIN
            srand(us_ticker_read());
            snprintf(setup_pin, sizeof(setup_pin), "%04d", rand() % 10000);
            pin_verified = false;
            char pmsg[64];
            snprintf(pmsg, sizeof(pmsg), "  *** NEW SETUP PIN: %s ***\r\n", setup_pin);
            pc_print(pmsg);
            ...
            if (oled_available) {
                ...
                oled_str_med_c(34, "Ember-Setup");
                char pl[18];
                snprintf(pl, sizeof(pl), "PIN: %s", setup_pin);
                oled_str_med_c(52, pl);
                oled_flush();
            }
            pc_print("[SoftAP] Waiting for WiFi provisioning...\r\n");
            { char junk[256]; while (esp.readable()) esp.read(junk, sizeof(junk)); }
        ...
        if (test_alarm_active) {
            ...
            for (int sl = 0; sl < 40; sl++) {
                poll_button();
                if (btn_pressed) break;  // button pressed, handle immediately
                ThisThread::sleep_for(50ms);
            }
        }
    }
